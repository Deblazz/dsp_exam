...
var WSMessage = require('../components/ws_message');
  ...
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [film.title, owner, film.private, film.watchDate, film.rating, film.favorite], function (err) {
      if (err) {
        ...
        // send a retained message to the MQTT broker for the newly created film (only if it is public)
        if (!film.private) {
          var message = new MQTTMessage("inactive", null, null);
          mqtt.publishFilmMessage(this.lastID, message);
        }
