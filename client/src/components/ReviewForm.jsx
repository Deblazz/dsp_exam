...
const ReviewForm = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (reviewDate != "" && rating != undefined && review != undefined) {
      ...
      if (props.currentDraft) {
        props.submitNewDraft(review).then(completeReview).catch(() => { });
      } else {
        completeReview();
      }
    }
  ...
  return (
    <Form className="block-example border border-primary rounded mb-0 form-padding" onSubmit={handleSubmit}>
      ...
      {
        props.currentDraft &&
        <Button className="mb-3" variant="secondary" onClick={handleSaveDraft}>Save Draft</Button>
      }
      &nbsp;
      <Link to={nextpage}>
